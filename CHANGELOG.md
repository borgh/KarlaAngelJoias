...
### Fixed
- Alça de girar o cartão movida de cima pra baixo (não atrapalha mais as bolinhas do dedo) — bug real corrigido no processo: a nova posição saiu da área com `overflow-hidden` do container da foto, ficando invisível e inclicável ao mesmo tempo (clique vazava pro elemento seguinte da página). Corrigido separando o corte de cantos arredondados (só na foto) da área de referência dos controles (sem corte). `[78b42a9]`
- Cartão confirmado certo na checagem ao vivo (indicador verde) ainda saía errado depois de capturar — o refinamento pós-captura (sabidamente imperfeito) rodava sempre, mesmo por cima de uma posição já validada. Agora, quando a checagem ao vivo já confirmou, usa essa posição diretamente, sem rodar refinamento nenhum por cima. Instrução das bolinhas esclarecida: o centro da cruz é o ponto exato. `[cb8fa07]`
